...
- **Two view modes (`VIEW` global, persisted to `localStorage['cb_view']`; default `staff`):**
  - **Staff view** — for non-technical staff. Repos are grouped into purpose **categories** with plain-language summaries; cards lead with an **"Open the tool"** action and the modal (`renderStaffRepoModal`) is a lightweight info card. No clone command, file tree, or code views. Chip row shows **category** chips; the Sort control is hidden.
  - **Developer view** — the original experience, unchanged: flat grid (`renderRepos`/`buildRepoCard`), **language** chips, Sort, clone command, and the full Explore modal (file tree / README source / `index.html` preview).
  - `applyFilters()` branches on `VIEW`; `renderChips()` dispatches to `buildCategoryChips` (staff) or `buildLangChips` (dev); the one delegated chip handler (`wireChipRow`) reads `data-cat` **or** `data-lang`. `localStorage` holds only this UI pref — **never** the OAuth token.
- **Purpose categories (hybrid source).** `CATEGORIES` (ordered: authoring, reporting, translation, tools, web, other) defines the on-page sections. `categoryForRepo(repo)` resolves a repo's category by: (1) a GitHub repo **topic** matching a category id or `cat-<id>`/`category-<id>`; (2) the curated **`CURATED`** map (repo-name → `{category, summary}`); (3) the `other` bucket. **When you add a project:** set a `cat-<id>` topic on the repo on GitHub *or* add a line to `CURATED`. Summaries are plain/factual (not marketing — see the no-marketing rule above).
